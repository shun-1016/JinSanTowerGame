...
v20.3は、**12.pngで三角形分割に失敗して矩形へフォールバックする問題を調査する診断版**です。
...
## v20.3 物理ポリゴン軽量化
- v20.1で確立したアルファ輪郭・画像/物理座標同期は維持。
- 三角形化後、隣接する三角形を「結合後も凸ポリゴンになる場合」に順次結合。
- 物理Bodyは元の約200個の三角形ではなく、より少数の凸ポリゴンCompound Bodyで構成。
- 外周は元の輪郭を維持するため、見た目と物理形状の外形を大きく変えない。
- デバッグ表示では「元三角形」と「物理凸ポリゴン」を分けて表示。
