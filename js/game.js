...
(function () {
  ...
  const Game = {
    pieces: [], current: null, queue: [], score: 0, gameEnded: false, cameraY: 0, nextId: 1,
    spawnGap: 45,
    async init() {
      this.pieces=[]; this.queue=[]; this.score=0; this.gameEnded=false; this.cameraY=0; this.nextId=1;
    ...
    spawn() {
      ...
      const x=Renderer.width/2, y=this.cameraY+70;
    ...
    rotate(dir) {
      const p=this.current; if(!p || p.falling || this.gameEnded) return;
      Body.rotate(p.body, dir * Math.PI/12);
    },
    drop() {
      const p=this.current; if(!p || p.falling || this.gameEnded) return;
      Body.setStatic(p.body,false); Body.setVelocity(p.body,{x:0,y:0}); Body.setAngularVelocity(p.body,0); p.falling=true;
    },
    update(dt) {
      if(this.current && this.current.falling) {
        const b=this.current.body;
        // Once Matter.js puts the body to sleep, it is considered fixed naturally.
        if(b.isSleeping) this.fixCurrent();
        // Keep the active piece within the playable horizontal area.
    ...
    fixCurrent() {
      ...
      // Wake is unnecessary; a sleeping body remains part of the tower.
    ...
    updateCamera() {
      ...
      if(!all.length){this.cameraY=0;return;}
      let top=Infinity; for(const p of all) top=Math.min(top,p.body.bounds.min.y);
      const target=Math.max(100,Renderer.height*.25);
