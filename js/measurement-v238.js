/* v23.8 - all-piece collision diagnostics / no physics stabilization */
(() => {
  'use strict';
  ...
  const state = {
    ...
    stageW: 390, stageH: 500, baseWidth: 0, body: null, running: false, raf: 0
  };
  ...
  function startPiece(index){
    ...
    state.index=index; state.frame=0; state.startedAt=performance.now(); state.landingFrame=null; state.rows=[]; state.body=p.body;
  }
  ...
  function tick(){
    ...
    if(state.landingFrame!==null && state.frame-state.landingFrame>=POST_LAND_FRAMES){
      ...
      if(state.index+1<state.images.length){
        setStatus(`v23.8 計測中: ${state.index+2}/${state.images.length}`); startPiece(state.index+1);
  ...
  function finishRun(){
    state.running=false; cancelAnimationFrame(state.raf); clearDynamicBodies();
  ...
  function start(){
    ...
    state.run=n; state.index=0;state.frame=0;state.rows=[];state.allRows=[];state.summaries=[];state.running=true;
